  ...
  (let [raw-code (slurp file)
    ...
    (if (skip-file? raw-code)
      (do
        ...
        (spit out-file raw-code))

      (let [{:keys [new-source registry]}
            (try
              (rewrite-lambdas raw-code)
              (catch Throwable e
                (throw
                 (ex-info
                  (str "lambda-lifter failed parsing/rewriting: " file-path)
                  {:file file-path
                   :stage :rewrite-lambdas}
                  e))))

            registry-code
            (generate-registry-code registry)
            ...
            ast-form
            (try
              (read-cljs wrapped-text)
              (catch Throwable e
                (let [debug-file
                      (io/file
                       (str (.getPath out-file) ".lambda-lifter-error.cljs"))]
                  (io/make-parents debug-file)
                  (spit debug-file wrapped-text)

                  (throw
                   (ex-info
                    (str "lambda-lifter generated invalid CLJS for: "
                         file-path
                         "\nGenerated source dumped to: "
                         (.getPath debug-file))
                    {:file file-path
                     :output-file (.getPath out-file)
                     :debug-file (.getPath debug-file)
                     :stage :read-generated-source}
                    e)))))

            instrumented
            (instrument-host-file ast-form [])]
